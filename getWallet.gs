/*====================================================================================================================================*
 * Импорт балансов кошельков крипто-бирж:
 *    EXMO
 *    BITTREX
 *    BITFINEX
 *
 * Возвращает двухмерный массив с балансами кошелька биржи. Для Биттрекс и Битфайнекс просто парсится ответ, для Эксмо полученный
 * массив очищается от лишней информации и переворачивается на 90 градусов, верхняя половина строк - доступный баланс, 
 * нижняя половина - сумма резерва (например в ордерах).
 *
 * Параметры функции:
 *
 *    key:     публичный ключ API биржи 
 *    secret:  секретный ключ API биржи
 *    exname:  название в нижнем регистре, поддерживаются только "exmo", "bittrex", "bitfinex"
 *
 * Примеры использования:
 *
 *   =getWallet(B1;B2;"bitfinex")  // в ячейках B1 и B2 ваши ключи от API 
 *
...
function getWallet(key,secret,exname) {
  ...
  switch (exname){
    case 'exmo': 
     // var baseUrl = "http://api.exmo.com/v1/user_info"; 
    ...
    case 'bittrex':
      ...
      var payload = baseUrl.concat("?apikey=" + key + "&nonce=" + nonce); 
      var uri = payload;
      break;
  ...
  switch (exname){
    ...
    case 'bittrex':
     ...
     case 'bitfinex':
       var Sign = Utilities.computeHmacSignature(Utilities.MacAlgorithm.HMAC_SHA_384, payload, secret);    
  }     
      
  Sign = Sign.map(function(byte) {
  ...
  switch (exname){
    case 'exmo':    
      var ndata = []; 
      var data = parseJSONObject_(JSON.parse(jsondata.getContentText(),","),"/balances", "noInherit,noTruncate,replaceDot",includeXPath_, defaultTransform_);     
      ...
      data = parseJSONObject_(JSON.parse(jsondata.getContentText(),","),"/reserved", "noInherit,noTruncate,replaceDot",includeXPath_, defaultTransform_);  
      
      for(var i=0, w = data.length; i<w ;i++ )       
        for(var l=0, y = data[i].length ; l<y ;l++ ){
          if (!ndata[l+j])
            ndata[l+j] = [];  
          ndata[l+j][i] = data[i][l];           
        }   
    ...
    case 'bittrex':
    case 'bitfinex':
      return  parseJSONObject_(JSON.parse(jsondata.getContentText()),"/", "noInherit,noTruncate,replaceDot",includeXPath_, defaultTransform_); 
  }  
}
...
//=====================================================================================================================================//
//=====================================================================================================================================//
//   ImportJSON by Brad Jasper and Trevor Lohrbeer удобная библиотека для запроса данных через API например запрос курса BTC к USD:    //
//                                                                                                                                     //
//   =importJSON("http://api.exmo.me/v1/order_book/?pair="&A3;"/"&A3&"/ask_top";"noInherit,noTruncate,noHeaders,replaceDot";$C$1)      //
//  где А3 ячейка содержащая название валютной пары, например "BTC_USD"                                                                //
//                                                                                                                                     //
//  от авторской отличается опцией "replaceDot" которая позволяет заменить точку на запятую без танцев с бубном в таблице              //
//                                                                                                                                     //
//=====================================================================================================================================//
//=====================================================================================================================================//
 ...
 **/
function ImportJSON(url, query, parseOptions, sw) {
 ...
 *    noHeaders:     Don't include headers, only the data
 *    debugLocation: Prepend each value with the row & column it belongs in
 *    replaceDot:    Replacement point to a comma
 *
 * For example:
 *
...
function defaultTransform_(data, row, column, options) {
  ...
  if (hasOption_(options, "replaceDot")) {
    data[row][column] = data[row][column].toString().replace('.',','); 
  }
}
